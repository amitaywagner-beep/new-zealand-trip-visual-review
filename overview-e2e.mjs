...
await test('mobile-overview-structure',async()=>{
 ...
 const info=await o.evaluate(el=>{const er=el.getBoundingClientRect(); return ({
   height:er.height,width:er.width,
   buttons:[...el.querySelectorAll('button,a,[role="button"]')].map(x=>{const r=x.getBoundingClientRect(),s=getComputedStyle(x);const ix=Math.max(0,Math.min(r.right,er.right)-Math.max(r.left,er.left));const iy=Math.max(0,Math.min(r.bottom,er.bottom)-Math.max(r.top,er.top));return{text:(x.innerText||x.textContent||'').trim(),x:r.x-er.x,y:r.y-er.y,width:r.width,height:r.height,fontSize:s.fontSize,visibleArea:ix*iy,area:r.width*r.height,mostlyVisible:(ix*iy)>=(r.width*r.height*0.8),overflowX:x.scrollWidth>x.clientWidth,overflowY:x.scrollHeight>x.clientHeight}}).filter(x=>x.text),
...
await test('mobile-overview-stop-06-clicks-map',async()=>{
 ...
 const after=await p.evaluate(()=>scrollY); const map=p.locator('.leaflet-container').first(); const mapVisible=await map.isVisible().catch(()=>false); const body=await p.locator('body').innerText(); if(!mapVisible||!/ניו פלימות|New Plymouth/i.test(body)) throw new Error('map did not focus stop 06');
...
await test('desktop-overview-palmerston-clicks-map',async()=>{
 const ctx=await browser.newContext({viewport:{width:1440,height:1000}}); const p=await ctx.newPage(); await p.goto(BASE+'/visual-review/pages/trip-at-a-glance',{waitUntil:'domcontentloaded',timeout:60000}); await p.waitForTimeout(1200);
 const o=p.locator('.route-overview-graphic').first(); const candidates=o.locator('button,a,[role="button"]'); const n=await candidates.count(); let target=null;
 ...
 if(!target) throw new Error('Palmerston control not found'); await target.click(); await p.waitForTimeout(900); const body=await p.locator('body').innerText(); const mapVisible=await p.locator('.leaflet-container').first().isVisible().catch(()=>false); if(!mapVisible||!/Palmerston North|פלמרסטון/.test(body)) throw new Error('Palmerston did not focus map');
